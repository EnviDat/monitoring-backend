...
def get_nead_string(db_fields_list, nead_dict):
    ...
    for item in db_fields_list:
        if item in nead_dict:
            ...
            print(
                f'WARNING (nead_header_strings.py) "{item}" not a valid key in {nead_dict}'
            )
            return
...
def get_fields_string(database_fields_list):
    fields_dict = {
        ...
        "swout": "OSWR",
        "swin_stdev": "ISWR_stdev",
        "netrad": "NSWR",
        "netrad_stdev": "NSWR_stdev",
        "airtemp1": "TA1",
...
def get_database_fields_data_types_string(database_fields_list):
    ...
    display_units_string = get_nead_string(
        database_fields_list, database_fields_data_types_dict
    )
...
def get_display_description(database_fields_list):
    display_description_dict = {
        ...
        "swin_stdev": "shortwave_incoming_radiation_stdev",
        "netrad": "net_radiation",
        "netrad_stdev": "net_radiation_stdev",
    ...
    display_description_string = get_nead_string(
        database_fields_list, display_description_dict
    )
