...
def get_station_id(model, stations_config):
    station_id_dict = {
        stations_config.get(s, "model_url", fallback=""): s
        for s in stations_config.sections()
        if s != "DEFAULT"
    }
...
def write_nead_config(
    config_path, model, stringnull="", delimiter=",", ts_meaning="end"
):
    ...
    try:
        ...
        database_fields_data_types_string = get_database_fields_data_types_string(
            database_fields_list
        )
        config.set(
            "FIELDS", "database_fields_data_types", database_fields_data_types_string
        )
        ...
        # Print error message
        print(
            f"WARNING (write_nead_config.py): could not write nead header config, EXCEPTION: {e}"
        )
        return None, None
