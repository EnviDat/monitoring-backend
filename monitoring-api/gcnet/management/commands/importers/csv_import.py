...
class CsvImporter:
    ...
    def logger_csv(
        self, source, input_file, output_file, config, header=DEFAULT_HEADER
    ):
                    ...
                    if len(rows_buffer) > rows_after:
                        ...
                        sink.write(
                            ",".join(
                                [
                                    f"{v}"
                                    for v in rows_buffer[-(1 + rows_after)].values()
                                ]
                            )
                            + "\n"
                        )
            # save the last line #TODO: Depending on rows after
            sink.write(",".join([f"{v}" for v in rows_buffer[-1].values()]) + "\n")
    ...
    def _clean_csv_line(self, row):
        row = {
            ...
            Columns.SWOUT_MIN.value: row["SWoutMin"],
            Columns.NETRAD_MAX.value: row["NetRadMax"],
