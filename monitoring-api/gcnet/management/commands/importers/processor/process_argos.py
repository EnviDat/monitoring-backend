...
def read_argos(file, nrows):
    ...
    skip_rows = get_search_strings_line_numbers(
        file,
        [
            "/Invalid day of the month: {0}: begin date is posterior to the last day of the year",
            "ARGOS READY",
        ],
    )
    ...
    df.loc[df["Station"].notnull(), "Station"] = df.loc[
        df["Station"].notnull(), "Timestamp"
    ].str[0:6]
    ...
    df = df.set_index(
        columns_names[:2] + ["timestamp_group"] + ["timestamp_row"]
    ).unstack()
...
def decode_argos(df, remove_duplicate=True, sort=True):
    ...
    if remove_duplicate:
        df = df.drop_duplicates(
            subset=["Station"] + [f"v_{i}" for i in range(1, 17)], inplace=False
        )
    ...
    if sort:
        ...
        # TEST
        df = df.sort_values(
            by=["Year", "Month", "Day", "Station", "Hours", "Minutes", "Seconds"],
            ascending=True,
        )
