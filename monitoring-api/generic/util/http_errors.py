...
def model_http_error(model):
    return HttpResponseNotFound(
        "<h1>Page not found</h1>" "<h3>Non-valid model: {}</h3>".format(model)
    )
...
def parent_class_http_error(parent_class):
    return HttpResponseNotFound(
        "<h1>Page not found</h1>"
        "<h3>Non-valid 'parent_class': {}</h3>".format(parent_class)
    )
...
def parameter_http_error(parameter):
    return HttpResponseNotFound(
        "<h1>Page not found</h1>"
        "<h3>No valid parameter(s) entered in URL: {}</h3>".format(parameter)
    )
...
def timestamp_meaning_http_error(timestamp_meaning):
    return HttpResponseNotFound(
        "<h1>Page not found</h1>"
        "<h3>Non-valid 'timestamp_meaning' kwarg entered in URL: {}</h3>"
        "<h3>Valid 'timestamp_meaning' kwarg options: end, beginning".format(
            timestamp_meaning
        )
    )
