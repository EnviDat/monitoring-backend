...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            migrations_commands = [
                f"python manage.py makemigrations {app} --noinput",
    ...
    def get_model_path(parent_class):
        ...
        if parent_class in ["LWFMeteo", "LWFStation"]:
            ...
            raise Exception(
                f"ERROR {parent_class} parent_class does not exist or does not have model_path specified "
                f"in new_model.py"
            )
    ...
    def get_app(parent_class):
        ...
        if parent_class in ["LWFMeteo", "LWFStation"]:
            ...
            raise Exception(
                f"ERROR {parent_class} parent_class does not exist or does not have app specified in "
                f"new_model.py"
            )
