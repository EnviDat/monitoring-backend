...
# TEST:      python manage.py dat_import -s test -c config/stations.ini -i gcnet/data/01c.dat -d gcnet/data -m test

# SWISS CAMP 10m:  Unable to run, this station has a different .dat file format with a header
# SWISS CAMP:      python manage.py dat_import -s 01_swisscamp -c config/stations.ini -i gcnet/data/01c.dat -d gcnet/data -m swisscamp_01d
# CRAWFORD POINT:  python manage.py dat_import -s 02_crawfordpoint -c config/stations.ini -i gcnet/data/02c.dat -d gcnet/data -m crawfordpoint_02d
# NASA-U:          python manage.py dat_import -s 03_nasa_u -c config/stations.ini -i gcnet/data/03c.dat -d gcnet/data -m nasa_u_03d
# GITS:            python manage.py dat_import -s 04_gits -c config/stations.ini -i gcnet/data/04c.dat -d gcnet/data -m gits_04d
# HUMBOLDT:        python manage.py dat_import -s 05_humboldt -c /config/stations.ini -i gcnet/data/05c.dat -d gcnet/data -m humboldt_05d
# SUMMIT:          python manage.py dat_import -s 06_summit -c config/stations.ini -i gcnet/data/06c.dat -d gcnet/data -m summit_06d
# TUNU-N:          python manage.py dat_import -s 07_tunu_n -c config/stations.ini -i gcnet/data/07c.dat -d gcnet/data -m tunu_n_07d
# DYE-2:           python manage.py dat_import -s 08_dye2 -c config/stations.ini -i gcnet/data/08c.dat -d gcnet/data -m dye2_08d
# JAR-1:           python manage.py dat_import -s 09_jar1 -c config/stations.ini -i gcnet/data/09c.dat -d gcnet/data -m jar1_09d
# SADDLE:          python manage.py dat_import -s 10_saddle -c config/stations.ini -i gcnet/data/10c.dat -d gcnet/data -m saddle_10d
# SOUTHDOME:       python manage.py dat_import -s 11_southdome -c config/stations.ini -i gcnet/data/11c.dat -d gcnet/data -m southdome_11d
# NASA-EAST:       python manage.py dat_import -s 12_nasa_east -c config/stations.ini -i gcnet/data/12c.dat -d gcnet/data -m nasa_east_12d
# NASA-SOUTHEAST:  python manage.py dat_import -s 15_nasa_southeast -c config/stations.ini -i gcnet/data/15c.dat -d gcnet/data -m nasa_southeast_15d
# PETERMANN:       python manage.py dat_import -s 22_petermann -c config/stations.ini -i gcnet/data/22c.dat -d gcnet/data -m petermann_22d
# NEEM:            python manage.py dat_import -s 23_neem -c config/stations.ini -i gcnet/data/23c.dat -d gcnet/data -m neem_23d
# EAST-GRIP:       python manage.py dat_import -s 24_east_grip -c config/stations.ini -i gcnet/data/24c.dat -d gcnet/data -m east_grip_24d
...
from gcnet.datvalidator import null_checker
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        field_names = ['timestamp_iso', 'timestamp', 'year', 'julianday', 'quarterday', 'halfday', 'day', 'week',
                       ...
                       'battvolt', 'swin_max', 'swout_min', 'netrad_max',
        ...
        try:
            with open(writer_no_duplicates, 'w', newline='') as sink, open(input_file, 'r') as source:
                ...
                while True:
                    ...
                    line_clean = {'timestamp_iso': gcnet_utc_datetime(row['Year'], row['Doyd']),
                                  ...
                                  'swout_min': row['SWoutMin'], 'netrad_max': row['NetRadMax'],
            ...
            print('WARNING (dat_import.py) file not found {0}, exception {1}'.format(input_file, e))
            return

        if model_class is None:
            print('WARNING (dat_import.py) no data found for {0}'.format(kwargs['station']))
            return
        ...
        c = CopyMapping(
            ...
            dict(timestamp_iso='timestamp_iso', timestamp='timestamp', year='year', julianday='julianday',
                 ...
                 battvolt='battvolt', swin_maximum='swin_max', swout_minimum='swout_min', netrad_maximum='netrad_max',
                 airtemp1_maximum='airtemp1_max', airtemp2_maximum='airtemp2_max', airtemp1_minimum='airtemp1_min',
                 airtemp2_min='airtemp2_min', windspeed_u1_maximum='windspeed_u1_max',
                 windspeed_u2_maximum='windspeed_u2_max',
